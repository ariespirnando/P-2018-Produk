<script type="text/javascript">
     ...
     $('.nama_karyawan').keyup(function(e) {  
      var config = {
          source: function(request, response) {
              $.getJSON("<?php echo base_url() ?>/timbang/karyawan", { term: $('.nama_karyawan').val()}, 
                        response);
          },                     
     ...
     function loadPagination(pagno){ 
       ...
       $.ajax({
         url: '<?php echo base_url()?>timbang/loadRecord/'+pagno,
         type: 'post',
         data: "q="+q,
         dataType: 'json',
     ...
     function loaddetail(){ 
       $.ajax({
         url: '<?php echo base_url()?>timbang/loaddetailform',
         type: 'post', 
         success: function(response){
     ...
     for(index in result){  
        ...
        sno+=1;
        tr += "<td>"+ sno +"</td>";
        tr += "<td>"+ result[index].cNomor_timbang +"</td>"; 
     ...
     function simpanapp(){
      ...
      if(o>0){
        ...
        $.ajax({
         url: '<?php echo base_url()?>timbang/savedata',
         type: 'post', 
         data: datas,
         success: function(response){  
     ...
     function add_row2(table_id) {   
      ...
      var row_content = ''; 
        row_content  += '<tr>';
        row_content  += '    <td style="width:5%;text-align: center;"><span class="tabletimbang_numasd">'+c+'</span></td>';
        ...
        row_content  += '    <td style="width:10%;text-align: center;"><span onClick="del_row(this, \'tabletimbang\')" class="btn btn-danger"><span class="glyphicon glyphicon-trash" aria-hidden="false"></span></span></td>';
        row_content  += '</tr>'; 
      ...
      $(".angka_"+c).css('text-align','right'); 
       ...
       $('.nama_jenis_'+c).keyup(function(e) {  
          var config = {
              source: function(request, response) {
                  $.getJSON("<?php echo base_url() ?>/timbang/jenis", { term: $('.nama_jenis_'+c).val()}, 
                            response);
              },                     
              select: function(event, ui){
     ...
     $('.nama_jenis').keyup(function(e) {  
      var config = {
          source: function(request, response) {
              $.getJSON("<?php echo base_url() ?>/timbang/jenis", { term: $('.nama_jenis').val()}, 
                        response);
          },                     
          select: function(event, ui){
    ...
    function deletedata(i){ 
      var url = '<?php echo base_url()?>timbang/deleteid';
